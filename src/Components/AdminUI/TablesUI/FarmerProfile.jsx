...
import EditDialog from './FarmerDialogs/EditDialog';
import AddDialog from  './FarmerDialogs/AddDialog';
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (let i = 0; i < deletedRows.length; i++) {
        const response = await fetch(`http://localhost:5000/api/farmers/${deletedRows[i]._id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
