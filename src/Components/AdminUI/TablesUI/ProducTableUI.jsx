...
import EditDialog from './ProductsDialogs/EditDialog';
import AddDialog from  './ProductsDialogs/AddDialog';

import {Menu, MenuItem } from '@mui/material';



function createData(id, _id, DateAdded, ProductName, FarmerName,Price , Inventory, status, action) {
  return {
    ...
    >
      {numSelected > 0 ? (
        ...
        <>
          ...
          >
            Product List<br />
            <Typography variant="caption" sx={{ color: 'grey'}}>A Data View of Available Products for listing and verification</Typography>
          </Typography>
        </>
      )}
      {numSelected > 0 ? (
      ...
      />
         <Tooltip title="Add new Data">
            ...
            sx={{ minWidth: '180px', marginRight: 2, backgroundColor: '#0C7230', '&:hover': { backgroundColor: '#0C7230' } }}
            onClick={onAddClick}
            >
              <AddIcon sx={{ paddingRight: 0.6 }} /> Add Product</Button>
          </Tooltip>
    ...
    const fetchData = async () => {
      try {
        ...
        const formattedRows = data.map((item, index) =>
          ...
          createData(index + 1, item._id, item.DateAdded, item.ProductName, item.FarmerName, item.Price, item.Inventory, item.status, '')
        );
  ...
  const filteredRows = rows.filter(
    (row) =>
      (row.ProductName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        row.Inventory.toString().includes(searchQuery)) &&
  ...
  const handleAddFarmer = async (newOrder) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error adding products:', response.statusText);
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      for (let i = 0; i < deletedRows.length; i++) {
        const response = await fetch(`http://localhost:5000/api/products/${deletedRows[i]._id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
